/**
 * Class Craft.ExpressForms.SubmissionsTableView
 */
Craft.ExpressForms.SubmissionsTableView = Craft.TableElementIndexView.extend(
  {
    startDate: null,
    endDate: null,
    ...
    getStorage: function (key) {
      return Craft.ExpressForms.SubmissionsTableView.getStorage(this.elementIndex._namespace, key);
    },

    setStorage: function (key, value) {
      Craft.ExpressForms.SubmissionsTableView.setStorage(this.elementIndex._namespace, key, value);
    },

    createChartExplorer: function () {
      // chart explorer
      const $chartExplorer = $('<div class="chart-explorer"></div>').appendTo(this.$explorerContainer);
      const $chartHeader = $('<div class="chart-header"></div>').appendTo($chartExplorer);
      const $dateRange = $('<div class="date-range" />').appendTo($chartHeader);
      const $startDateContainer = $('<div class="datewrapper"></div>').appendTo($dateRange);
      const $endDateContainer = $('<div class="datewrapper"></div>').appendTo($dateRange);
      const $total = $('<div class="total"></div>').appendTo($chartHeader);
      $('<div class="total-label light">' + Craft.t('app', 'Total Submissions') + '</div>').appendTo($total);

      const $totalValueWrapper = $('<div class="total-value-wrapper"></div>').appendTo($total);
    ...
    handleStartDateChange: function () {
      if (
        this.setStartDate(Craft.ExpressForms.SubmissionsTableView.getDateFromDatepickerInstance(this.startDatepicker))
      ) {
        this.loadReport();
      }
    },
    ...
    handleEndDateChange: function () {
      if (this.setEndDate(Craft.ExpressForms.SubmissionsTableView.getDateFromDatepickerInstance(this.endDatepicker))) {
        this.loadReport();
      }
    },
    ...
    loadReport: function () {
      ...
      requestData.startDate = Craft.ExpressForms.SubmissionsTableView.getDateValue(this.startDate);
      requestData.endDate = Craft.ExpressForms.SubmissionsTableView.getDateValue(this.endDate);
      ...
      Craft.postActionRequest(
        'express-forms/reports/submissions-index',
        requestData,
  ...
  },
  {
    ...
    getStorage: function (namespace, key) {
      if (
        Craft.ExpressForms.SubmissionsTableView.storage[namespace] &&
        Craft.ExpressForms.SubmissionsTableView.storage[namespace][key]
      ) {
        return Craft.ExpressForms.SubmissionsTableView.storage[namespace][key];
      }
    ...
    setStorage: function (namespace, key, value) {
      if (typeof Craft.ExpressForms.SubmissionsTableView.storage[namespace] === typeof undefined) {
        Craft.ExpressForms.SubmissionsTableView.storage[namespace] = {};
      }

      Craft.ExpressForms.SubmissionsTableView.storage[namespace][key] = value;
    },
